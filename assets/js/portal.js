...
async function loadNotificationsPage(userId){
  const list=$("[data-notification-list]"),empty=$("[data-notification-empty]"),countEl=$("[data-notification-count]");if(!list)return;
  const render=async()=>{
    const{data,error}=await supabase.from("vmc_notifications").select("id,type,title,message,read_at,created_at,metadata").eq("member_id",userId).order("created_at",{ascending:false}).limit(50);
    if(error)throw error;const rows=data||[],unread=rows.filter(r=>!r.read_at).length;
    if(countEl)countEl.textContent=unread+" unread";list.replaceChildren();if(empty)empty.hidden=rows.length>0;
    rows.forEach(r=>{const item=document.createElement("article");item.className="notification-item"+(r.read_at?" is-read":"");item.dataset.notificationId=r.id;
      const main=document.createElement("div");const type=document.createElement("span");type.className="card-label";type.textContent=titleCase(r.type);const h=document.createElement("h3");h.textContent=r.title;const p=document.createElement("p");p.textContent=r.message;const time=document.createElement("time");time.dateTime=r.created_at;time.textContent=formatDateTime(r.created_at);main.append(type,h,p,time);
      const action=document.createElement("button");action.type="button";action.className="portal-button notification-read-button";action.textContent=r.read_at?"Read":"Mark read";action.disabled=!!r.read_at;action.addEventListener("click",async()=>{action.disabled=true;const{error}=await supabase.from("vmc_notifications").update({read_at:new Date().toISOString()}).eq("id",r.id).eq("member_id",userId);if(error){action.disabled=false;return}await render()});item.append(main,action);list.append(item);
    });
  };
  $("[data-mark-all-read]")?.addEventListener("click",async()=>{const{error}=await supabase.from("vmc_notifications").update({read_at:new Date().toISOString()}).eq("member_id",userId).is("read_at",null);if(!error)await render()});
  await render();
}
async function loadNotificationPreview(userId){
  const{data,error}=await supabase.from("vmc_notifications").select("title,message,read_at,created_at").eq("member_id",userId).order("created_at",{ascending:false}).limit(1).maybeSingle();if(error)throw error;
  const{count}=await supabase.from("vmc_notifications").select("id",{count:"exact",head:true}).eq("member_id",userId).is("read_at",null);
  $("[data-notification-preview-title]")?.replaceChildren(document.createTextNode(data?.title||"No new updates."));
